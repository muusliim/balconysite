...
const changeModalState = state => {
  ...
  function disabledBtn(btnSelector, num) {
    const btn = document.querySelector(btnSelector);
    if (Object.values(state).length < num) {
      btn.setAttribute('disabled', 'true');
    }
  }
  function activateBtn(btnSelector, num) {
    const btn = document.querySelector(btnSelector);
    if (Object.values(state).length == num) {
      btn.removeAttribute('disabled');
    }
  }
  disabledBtn('.popup_calc_button', 4);
  disabledBtn('.popup_calc_profile_button', 6);
  function bindActiontoElems(event, elem, prop) {
    elem.forEach((item, i) => {
      item.addEventListener(event, () => {
        ...
        console.log(state);
        activateBtn('.popup_calc_button', 4);
        activateBtn('.popup_calc_profile_button', 5);
      });
    });
  }
  bindActiontoElems('click', windowForm, 'form');
  bindActiontoElems('input', windowWidth, 'width', '.popup_calc_button');
  bindActiontoElems('input', windowHeight, 'heigth', '.popup_calc_button');
  bindActiontoElems('change', windowType, 'view_type');
  bindActiontoElems('change', windowProfile, 'window_profile');
};
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      ...
      postData('assets/server.php', formData).then(res => {
        ...
        (0,_modals__WEBPACK_IMPORTED_MODULE_1__.closeModalByTime)('.popup_calc_end', 5000);
      });
    });
  });
};
...
const images = () => {
  ...
  // imagePopup.style.cssText = 'justify-content: center; align-items: center; display: none;';
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   closeModalByTime: () => (/* binding */ closeModalByTime),
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('#timer', '2023-06-29');
